    ...
    async def chat(self, chat: ChatIn, user: User) -> tuple[str, int]:
        ...
        if not chat.conversation_id:
            conversation_obj = ConversationCreate(
                content=[], prompt_id=prompt.id, user_id=user.id
            )
            new_conversation = await self.conversation_service.create(conversation_obj)
            chat.conversation_id = new_conversation.id
        else:
            await self.conversation_service.get_for_user(chat.conversation_id, user.id)

        if chat.conversation_id is None:
            raise RuntimeError("No se pudo determinar la conversación activa.")
        ...
        try:
            ...
            if store:
                ...
                global_results = await store.asearch(
                    ("global", "verified"), query=chat.message, limit=2
                )
                ...
                if user.id:
                    ...
                    for r in user_results:
                        if "preference" in r.value:
                            memories.append(
                                f"- PREFERENCIA DE {user.id}: {r.value['preference']}"
                            )
            ...
            raise RuntimeError(
                f"Error accediendo al store para contexto adicional: {e}"
            ) from e

        final_prompt = f"{metadata}\\n{store_context}\\n{system_prompt}"

        response = await self.agent.answer_message(
            message=chat.message,
            system_prompt=final_prompt,
            conversation_id=chat.conversation_id,
            checkpoint_ns=user.id,
        )
        formatted_messages = format_message(user=chat.message, agent=response)
        await self.conversation_service.update_messages(
            chat.conversation_id, formatted_messages
        )
