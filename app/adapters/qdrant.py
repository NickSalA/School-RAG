...
async def connect_async_vectorial_client() -> AsyncQdrantClient:
    """Conectar al cliente de Qdrant.
    
    Returns:
        AsyncQdrantClient: Instancia del cliente de Qdrant asíncrono.
...
def connect_vectorial_client() -> QdrantClient:
    """Conectar al cliente de Qdrant (síncrono).
    Returns:
...
async def get_async_vector_store(client: AsyncQdrantClient, index_name: str) -> QdrantVectorStore:
    ...
    try:
        ...
        if not await client.collection_exists(index_name):
