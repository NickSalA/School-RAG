...
try:
    ...
    if (
        DATABASE_URL
        and not str(DATABASE_URL).startswith("sqlite")
        and settings.DATABASE_SSL_ENABLED
    ):
    ...
    async with AsyncSession(engine, expire_on_commit=False) as session:
        try:
            ...
            raise DatabaseIntegrityError(
                f"Violación de integridad en la BD: {e}"
            ) from e
            ...
            raise DatabaseQueryError(
                f"Error al ejecutar la consulta en la BD: {e}"
            ) from e
